...
export default async function AdminLayout({
  ...
  // account footer never fall out of sync between the two layouts.
  ...
  return (
    <AdminToastProvider>
      <div className="min-h-screen bg-parchment">
        ...
        <aside className="fixed inset-y-0 left-0 z-40 hidden h-screen w-64 flex-col border-r border-parchment/10 bg-ink lg:flex">
          {sidebarContent}
        </aside>
        ...
        <div className="flex flex-col lg:ml-64">
          {/* Top bar */}
          <header className="fixed inset-x-0 top-0 z-30 flex h-16 items-center justify-between border-b border-t-[3px] border-ink/10 border-t-brass bg-paper/80 px-4 backdrop-blur-md sm:px-8 lg:left-64">
            <div className="flex flex-1 items-center gap-3 sm:gap-4">
              <AdminMobileSidebarToggle>
                <div className="flex h-full flex-col bg-ink">{sidebarContent}</div>
              </AdminMobileSidebarToggle>
              <AdminTopbarTitle />
              <AdminSearch />
            </div>
